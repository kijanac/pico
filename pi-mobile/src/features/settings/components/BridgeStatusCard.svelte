...
<section class="rounded-[var(--radius-lg)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-3">
  <div class="mb-3 flex items-start justify-between gap-3">
    <div>
      <h2 class="text-[13px] font-medium text-[color:var(--color-fg)]">bridge</h2>
      {#if settingsState.bridgeUrlConfigured}
        <p class="mt-1 break-all text-[11px] leading-relaxed text-[color:var(--color-fg-faint)]">
    ...
    {#if settingsState.bridgeUrlConfigured}
      <div class="flex shrink-0 items-center gap-1.5 rounded-full border border-[color:var(--color-border)] px-2 py-1 text-[10px] text-[color:var(--color-fg-muted)]">
        {#if status === "checking"}
